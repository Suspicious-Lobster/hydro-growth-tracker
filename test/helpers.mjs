...
// `token` / `allowedOrigins` / `logger` are passed straight to createServer;
// when a token is set, every request from these helpers carries it unless
// `extraHeaders` overrides X-Hydro-Token (set it to '' to test the
// unauthenticated path).
export async function freshServer({ seed, extraHeaders = {}, token = null, allowedOrigins, logger } = {}) {
  ...
  const app = createServer({ dataFile, uploadsDir, token, allowedOrigins, ...(logger ? { logger } : {}) });
