...
const SettingsPanel = () => {
  ...
  const { effectsEnabled, muted, soundEnabled, remindersEnabled, voice, wanderEnabled, toggleEffects, setMuted, setSoundEnabled, setRemindersEnabled, setVoice, setWanderEnabled, resetDismissed, startTour } = useAssistant();
  ...
  return (
    ...
    <div className={`${colors.bgSecondary} rounded-xl shadow-lg p-6 ${colors.border} border space-y-4`}>
      ...
      <Toggle colors={colors} label="Bud wanders the screen" description="Off keeps him in his corner." checked={wanderEnabled !== false} onChange={() => setWanderEnabled && setWanderEnabled(!(wanderEnabled !== false))} />
